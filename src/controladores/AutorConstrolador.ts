...
export default class AutorControlador{
 ...
 detalhar(req:Request,res:Response){
    const{id} = req.params
    const autor = autores.find((elemnto)=>{
...
editar(req:Request,res:Response){
    //Buscar o Autor que vais er editado
    const{id} = req.params
    const{nome,idade} =req.body
 ...
 //o send envia mensagem sem nada no corpo do retorno     
  ...
  excluir(req:Request,res:Response){
    ...
    const autorIndice = autores.findIndex((elemnto)=>{
        return elemnto.id === id
    })
